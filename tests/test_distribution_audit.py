...
import spicetrellis.distribution_audit as distribution_audit
from spicetrellis.distribution_audit import (
...
def test_auditors_accept_canonical_regular_files() -> None:
    with tar_archive(
        [
            ("spicetrellis-1/", b"", tarfile.DIRTYPE),
            ("spicetrellis-1/src/", b"", tarfile.DIRTYPE),
            ("spicetrellis-1/src/module.py", b"value = 1\n", None),
        ]
    ) as archive:
        assert audit_sdist(archive, expected_root="spicetrellis-1") == {"src/module.py"}
    with zip_archive(
        [
            ("spicetrellis/", b"", stat.S_IFDIR | 0o755),
            ("spicetrellis/module.py", b"value = 1\n", stat.S_IFREG | 0o644),
        ]
    ) as archive:
        assert audit_wheel(archive) == {"spicetrellis/module.py"}
...
def test_sdist_can_require_the_exact_release_root() -> None:
    with (
        ...
        audit_sdist(archive, expected_root="spicetrellis-0.5.0")
